...
public static class EditorHelper
{
    ...
    public static void BeginUndoGruop(string undoName, TrafficHeadquarter trafficHeadquarter)
    {
        ...
        //headquarter에서 발생하는 모든 변화를 등록하게 됩니다.
    ...
    //레이와 구의 충돌 판별식임, 값이 True라면 구 반경에 레이가 hit 되었다는 뜻
    ...
    public static void CraeteLayer(string name)
    {
        ...
        {
            throw new ArgumentException("name", "새로운레이어를 추가할려면 이름을 꼭 입력해주세요.");
        }
        ...
        for(var i = 0; i < propCount; i++)
        {
            ...
            //buuiltin, 이미 다른 레이어가 자리를 차지하고 있다면
        ...
        if(firstEmptyProp == null)
        {
            Debug.LogError($"레이어가 최대 갯수에 도달하였습니다. 그래서 {name}을 생성하지 못했습니다.");
            return;
        }
